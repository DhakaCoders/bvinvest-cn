...
class google_business_reviews_rating_widget extends WP_Widget
{
    ...
    public function __construct()
    {
        ...
        parent::__construct($this->alias, __('Reviews and Rating'), array(
            'description' => __('Have your rating and a review showing in your sidebar'),
	...
	private function reviews_filter($filters = array(), $override = TRUE)
	{
		...
		$place_id = (isset($filters['place_id']) && is_string($filters['place_id']) && strlen($filters['place_id']) >= 20) ? $filters['place_id'] : NULL;
...
';
		if ($display_name)
		{
			$html .= '	<h3 class="business-name">' . (($display_icon && $this->business_icon != NULL) ? '<span class="icon"><img src="' . esc_attr($this->business_icon) . '" alt="' . esc_attr($this->business_name . ' ' . __('Icon')) . '"></span>' : '') . esc_html($this->business_name) . '</h3>
';
		}
		elseif ($display_icon && $this->business_icon != NULL)
		{
			$html .= '	<h3 class="business-icon icon"><img src="' . esc_attr($this->business_icon) . '" alt="' . esc_attr($this->business_name . ' ' . __('Icon')) . '"></h3>
';
		}
		...
		if ($display_rating)
		{
			...
			{
				$html .= '<span class="number-text">' . esc_html(__('Rating:')) . '</span> <span class="number">' . esc_html(number_format($this->rating, 1)) . '</span> ';
			}
...
';
		...
		if ($display_view_reviews_button || $display_write_review_button)
		{
			...
			{
				$html .= '<a href="' . esc_attr($view_reviews_url). '" target="_blank" rel="nofollow" class="button widget-button view-reviews">' . esc_html(__('View Reviews')). '</a>';
			}
			...
			{
				$html .= '<a href="' . esc_attr($write_review_url). '" target="_blank" rel="nofollow" class="button widget-button write-review">' . esc_html(__('Write Review')). '</a>';
			}
...
';
    ...
    public function form($instance)
    {
		...
		if (!$this->demo)
		{
			...
			{
				$html = '        <p class="error"><a href="' . esc_attr($this->plugin_settings_url) . '">'.esc_html(__('Please set your Google API Key and Place ID')).'</a>.</p>
        <p class="buttons"><a href="' . esc_attr($this->plugin_settings_url) . '" class="button button-secondary">' . esc_html(__('Settings')) . '</a></p>
';
			}
			elseif (!$this->api_key || $this->api_key == NULL)
			{
				$html = '        <p class="error"><a href="' . esc_attr($this->plugin_settings_url) . '">'.esc_html(__('Please set your Google API Key')).'</a>.</p>
        <p class="buttons"><a href="' . esc_attr($this->plugin_settings_url) . '" class="button button-secondary">' . esc_html(__('Settings')) . '</a></p>
';
			}
			elseif (!$this->place_id || $this->place_id == NULL)
			{
				$html = '        <p class="error"><a href="' . esc_attr($this->plugin_settings_url) . '">'.esc_html(__('Please set your Place ID')).'</a>.</p>
        <p class="buttons"><a href="' . esc_attr($this->plugin_settings_url) . '" class="button button-secondary">' . esc_html(__('Settings')) . '</a></p>
';
			}
			elseif ($this->result == NULL)
			{
				$html = '        <p class="error">'.esc_html(__('No rating or review data found.')).' <a href="' . esc_attr($this->plugin_settings_url) . '">'.esc_html(__('Please check your Rating and Reviews settings.')).'</a>.</p>
        <p class="buttons"><a href="' . esc_attr($this->plugin_settings_url) . '" class="button button-secondary">' . esc_html(__('Settings')) . '</a></p>
';
			}
		}
		...
		{
			$html = '        <p class="error">'.esc_html(__('Not reviews or ratings exist.')).'</p>
';
		}
